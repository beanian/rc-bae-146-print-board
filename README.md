...
1:15 scale, ~1800mm wingspan) in **LW-PLA**, finished in a **green / white livery**.
...
- **Livery colour layer** — each part carries a suggested filament colour for the green / white scheme:
  - 🟢 **Green LW-PLA** — the whole vertical fin (Rudder section) and the rear fuselage / tailcone wrap (`Fuse 9–11` + corners).
  - 🟠 **Two-tone** — `Fuse 8` sits on the diagonal white/green sweep (colour-change at Z, mask, or decal). Optional green accent on the engine exhausts (`EDF 2 L/R`).
  - ⚪ **White** — everything else (nose, mid fuselage, wings, flaps/ailerons, horizontal tailplane, gondolas).
  - Click any card's **colour swatch** to cycle white → green → two-tone and tune it to your own plan.

The green/white split follows the layout of the reference photo — the exact fuselage
